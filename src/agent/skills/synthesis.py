...
def build_strategy_signal_distribution(
    ...
    """Build the stable bullish/neutral/bearish display projection."""
    ...
    return {
        name: {
            "count": bucket["count"],
            "weight_share": round(bucket["weight"] / total_weight, 4) if total_weight > 0 else None,
        }
...
def derive_strategy_consensus_level(
    ...
    *,
    ...
    """Derive consensus using the authoritative multi-strategy thresholds."""
    if insufficient_evidence:
        return "insufficient"
    valid_opinions = [op for op in opinions if not op.invalid_signal]
    if len(valid_opinions) <= 1 or sum(op.confidence for op in valid_opinions) == 0:
        return "insufficient"
...
class StrategySynthesizer:
    ...
    def synthesize(
        ...
        *,
        ...
        supporting, opposing, primary_dissent = self._group_opinions(
            opinions, final_score, applied_weights
        )
        ...
        payload = {
            ...
            "signal_distribution": build_strategy_signal_distribution(opinions, applied_weights),
    ...
    def _group_opinions(
        ...
        if opposing_candidates:
            ...
            primary_dissent = _opinion_to_item(
                selected_opinion, applied_weight=selected_weight
            )
    ...
    def _aligned_weights(
        ...
        """Align safe display weights; direct callers fall back to confidence."""
        aligned: List[float] = []
        for index, opinion in enumerate(opinions):
            raw_weight: Any = (
                opinion.confidence
                if weights is None
                else weights[index] if index < len(weights) else 0.0
            )
            try:
