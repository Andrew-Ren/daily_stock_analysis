...
- `index.daily` 额外区分 `cn.exchange` 与 `cn.csi` coverage：沪深交易所指数使用固定多源链，CSI 指数按运行时契约只认 AkShare；CN realtime 只接受运行时实际有 handler 的 source token，Tushare realtime 能力仅声明 A 股。
